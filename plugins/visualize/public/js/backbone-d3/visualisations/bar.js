        ...
        View:Backbone.d3.PlotView.extend(
            {
                ...
                datamap:function(datapoint){
                    return {x:datapoint.get('x'), y:datapoint.get('y')};
                },
                plotdata:function () {
                    var data = this.collection.map(this.datamap, this);
                ...
                plot:function (options, data) {
                    // Copy these data to avoid closure issues below
                    var w = this.w;
                    var h = this.h;

                    var scale = h / _.max(data,function (d) {
                        return d.y;
                    }).y;

                    var yval = function (d) {
                        return h - scale * d.y - .5;
                    };
