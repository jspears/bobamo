...
var DocView = B.View.extend({
    ...
    className:'swagger',
    events:{
        'click #swagger-ui-container a':'onLink',
...
//                    $el.find('.response_throbber').attr('src', 'jsonschema/js/swagger-ui/images/throbber.gif')
    ...
    render:function (opts) {
        if (!config.url) {
            var self =this;
            require(['views/configure/jsonschema'], function (ConfigView) {
                var confView  = self.configView = new ConfigView();
                confView.render();
                var loc = window.location;
                confView.form.fields.url.setValue( loc.protocol+'//'+loc.host );
                self.$el.append('<h3>No URL defined, please configure this plugin</h3>')
                var $view = $('<div class="config"></div>')
                self.$el.append($view);

                $view.html(confView.el);
                confView.on('save-success', function(){
