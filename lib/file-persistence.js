...
FilePersistence.prototype.saveAll = function(conf, callback){
    ...
    var sha = crypto.createHash('sha1').update(JSON.stringify(gconf, replacer, 3)).digest('base64');

    fs.writeFile(this.filename, JSON.stringify(gconf, replacer, 3), 'utf-8', function(err, stuff){
        if (err){
