...
FilePersistence.prototype.save = function (key, data, callback) {
    ...
    var aconf = (conf.plugins.appeditor || (conf.plugins.appeditor = {}));

    if (aconf.modified)
        (aconf.last_modified ||(aconf.last_modified =  [])).push(aconf.modified )
    aconf.modified = new Date(now);

    var conf_str = JSON.stringify(conf, null, 3);
