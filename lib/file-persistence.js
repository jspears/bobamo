...
FilePersistence.prototype.saveAll = function(conf, callback){
    ...
    //to recalculate delete sha from hash. Probably won't work because the order will be wrong.
    var sha = crypto.createHash('sha1').update(JSON.stringify(gconf)).digest('base64');

    fs.writeFile(this.filename, JSON.stringify(gconf, null, 3), 'utf-8', function(err, stuff){
        if (err){
