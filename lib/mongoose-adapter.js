    ...
    function _field(p, path) {
        ...
        if (p.instance == 'ObjectID') {
            if (options.ref) {
                ...
                _u.extend(defaults, {
                    url:options.api + opts.ref + '?transform=labelval',
                    dataType:'String',
                    type:'Select',
                    mode:'single'
                });
            ...
            _u.extend(defaults, {
                url:options.api + p.ref + '?transform=labelval',
                dataType:'String',
                type:'Select',
                mode:'single'
            });
            ...
            if (modelName) {
                _u.extend(defaults, {
                    dataType:'Array',
                    url:options.api + modelName + '?transform=labelval',
                    type:'MultiEditor'
                });
            } else {
...
//        (defaults.validator ? defaults.validator : (defaults.validator = [])).push('required');
    ...
    function filterRo(v) {
        return 0 > options.read_only.indexOf(v);
    }
