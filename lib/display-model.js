...
Model.prototype.pathFor = function (path) {
    var ret = util.depth(this.paths, path.replace(/\./, '.subSchema.'));
    console.log('ret', path, ret && ret.title);
    return ret;
}
...
Model.prototype.fieldsFor = function (list_type) {
    ...
    _u(list_type).each(function (k, i) {
        ret[k] = util.depth(this.paths,k.replace(/\./g, '.subSchema.'));
    }, this);
    return ret;
};
...
Model.prototype.schemaFor = function onCreateSchema(fields) {
    ...
    _u(flatFields).each(function(v,k){
        util.depth(schema, k.replace(/\./g, '.subSchema.'), v, true);
    })
    return schema;
};
...
function Field(path, args) {
    this.path = path;
    ['dataType', 'help', 'url', 'type', 'options'].forEach(easyget(args), this);
    this.__defineGetter__('subSchema', function () {
        ...
        if (subSchema && this.type == 'Object'){
            ...
            console.log('subSchema - ', JSON.stringify(ret));
            return Object.keys(ret).length && ret;
        }
    });
    this.__defineGetter__('title', function () {
        var title = find('title', args);
        if (!title) {
            return inflection.titleize(inflection.humanize(this.path));
        }
    });
}
