...
function Model(modelName, args, nofinder) {
    ...
    _u.each(args, function (v, k) {
        if (!v) return;
        _u.each(v.paths || v.schema, function onPathArgs(vv, kk) {
...
var Finder = function (v, k, model) {
    ...
    this.model = new Model([model.modelName, 'finder',this.name].join('.'), [this.display, model], true);
}
...
Model.prototype.asModel = function onModel(fields){
