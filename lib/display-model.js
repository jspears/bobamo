...
function Model(modelName, args, nofinder) {
    ...
    this.__defineGetter__('schema', subSchemaMap('',args));
...
function subSchemaMap(path, args){
    return function(){
        ...
        var subSchema = (findAll('subSchema', args) || []).concat((findAll('schema', args) || []));
...
function Field(path, args, parent) {
    this.path = path;
    args = (Array.isArray(args) ? args : [args]).map(fixType, this);
