var _u = require('underscore'), inflection = require('./inflection'), util = require('./util'),
    ...
    easyget = util.easyget, find = util.find,
    findAll = util.findAll, global_options = require('./globals');
...
function Model(modelName, args, nofinder) {
    ...
    _u.each(args, function (v, k) {
        if (!v) return;
        _u.each( v.schema, function onPathArgs(vv, kk) {
    ...
    ['help','description', 'buttons', 'events', 'defaults'].forEach(easyget(args), this);
}
...
var Finder = function (v, k, model) {
    ...
    this.model = new Model(model.modelName, [this.display], true);
}
...
function Field(path, args, parent) {
    ...
    this.__defineGetter__('subSchema', function () {
        ...
        if (subSchema && (this.type == 'Object' || this.type == 'List')) {
            var ret = {}
