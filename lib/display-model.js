...
Model.prototype.editorsFor = function onEditorsFor(fields, schema) {
    ...
    _u(flat).each(function (k, v) {
        ...
        if (type && (global_options.builtin_editors.indexOf(type) < 0))
            editors[inflection.hyphenize(type)] = true;


    }, this);

    return Object.keys(editors).map(function(v){ return 'libs/editors/'+v});
}
...
function Field(path, args) {
    this.path = path;
    ['dataType', 'help', 'url', 'type', 'subSchema'].forEach(easyget(args), this);
