...
function Model(modelName, args, nofinder) {
    ...
    this.__defineGetter__('labelAttr', function onLabelAttrGet() {
        ...
        var labels = ['label', 'name', 'title', 'username'];
        for (var i = 0, l = labels.length; i < l; i++) {
            if (~this.fields.indexOf(labels[i]))
                return labels[i];
        }
...
//        }
    ...
    ['help', 'description', 'buttons', 'events', 'defaults', 'url', 'urlRoot', 'methods', 'summary', 'transactional'].forEach(easyget(args), this);
}

function noHidden(v) {
    return !v && v.display && v.display.hidden
}
function propertyMap(v){
    return v.property || v;
}
Model.prototype.fieldsetsFor = function(view){
    var fieldsets = this[view] || this.fieldsets;
    var title = this.title;
    var fields = [{legend:title, fields:[]}];
    fieldsets.forEach(function(v,k){
        if (_.isString(v)){
            fields[0].fields.push(v);
        }else{
            var cur  = fields[k] || (fields[k] = {});
            if (v.fields){
              cur.fields = v.fields.map(propertyMap);
              cur.legend = v.legend || title + (k > 0 ? ' '+k : '');
            }
        }
    })
    return fields;
...
Model.prototype.fieldsFor = function (list_type) {
    ...
    _u(list_type).each(function (k, i) {
        var key = k.property || k.path || k;
        var ks = toSubSchema(key);
        var ds = util.depth(this.schema, ks);
        ret[key] = _u.extend({}, ds, k);
    }, this);
    return ret;
};
