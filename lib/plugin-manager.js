...
var PluginManager = function (options, express) {
    ...
    _u.each(this.plugins,function(v,k){
        this.loadedPlugins[v.name] = v;
    }, options);
    // on initializaiton appModel doesn't contain modelPaths, editors or models when it creates a new DisplayModel
    this.appModel = new DisplayModel(this.options);
    // each plugin can contribute to the global AppModel
    // the AppModel contains editors, models, modelpaths and other global metadata
    this.loadAppModels();
    this.loadFilters();
    this.loadRoutes();
    this.configure();
...
PluginManager.prototype.loadAppModels = function () {
    this.plugins.forEach(function (plugin) {
        //for the MongoosePlugin, adds the appModel containing the mongoose models to the plugin manager globally
...
PluginManager.prototype.updateSchema = function(plugin, modelName, backboneSchema, callback){
    ...
    plugin = this.loadedPlugins[plugin] || plugin;
