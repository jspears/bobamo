...
PluginManager.prototype.configure = function (cb) {
    var obj = this.persist.read(this.configFile);
    var plugins = obj && obj.plugins || {};
    var reconfigure = this.reconfigure = [];
    var all = [];
    this.forEach(function (plugin) {
       all.push(Q.ninvoke(plugin, 'configure', plugins[plugin.name]).fail(function(){
    ...
    Q.allResolved(all)
        .done(function(){
            if (reconfigure.length){
                var token = this.setupToken = this.gentoken();
                console.warn("\n\n\n System Setup Token: "+token)
            }
            cb(reconfigure.length ? reconfigure : null);
        }.bind(this));
}
/**
