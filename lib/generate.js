...
//    console.log('factory',options.displayFactory);
    ...
    app.get(base + '/:admin?/js/:super?/views/:type/:view', function (req, res, next) {
        //     res.contentType('application/javascript');
        res.render(j('generate', 'views', req.params.view), makeOptions(req));
    });
    app.get(base + '/:admin?/js/:super?/:clazz/:type', function (req, res, next) {
        //     res.contentType('application/javascript');
    ...
    app.get(base + '/:admin?/templates/:super?/:type/:view', function (req, res, next) {
        //   res.contentType('text/html; charset=utf-8');
    ...
    function makeOptions(req) {
        ...
        if (req.params.type) {
            ...
            isAdmin = isAdmin || type == 'admin';
            var f = isAdmin ? editFactory : factory;
            if (isAdmin){

                opts.modelData = f.data;
                console.log('opts.modelData',opts.modelData);
            }
    ...
    function createEditors(model){
            ...
            _u.each(CSchema, function (v, k) {
                ...
                if (default_editors.indexOf(v.type) > -1)
                    def = true;
                else
